...
{
    public class VoucherApi23Controller : BaseController
    {
        ...
        public ActionResult UploadFileSendStore()
        {
            List<VOUCHER_API> list_product = new List<VOUCHER_API>();
            return View(list_product);
        }
        [HttpPost]
        public ActionResult UploadFileSendStore(FormCollection collection)
        {
            List<VOUCHER_API> list_product = new List<VOUCHER_API>();
            try
            {
                ...
                if ((file != null) && (file.ContentLength > 0) && !string.IsNullOrEmpty(file.FileName))
                {
                    ...
                    using (var package = new ExcelPackage(file.InputStream))
                    {
                        ...
                        for (int rowIterator = 2; rowIterator <= noOfRow; rowIterator++)
                        {
                            ...
                            try { code = workSheet.Cells[rowIterator, 1].Value.ToString(); } catch (Exception) { code = ""; }
                            try { type = workSheet.Cells[rowIterator, 2].Value.ToString(); } catch (Exception) { type = ""; }
                            try { agent = workSheet.Cells[rowIterator, 3].Value.ToString(); } catch (Exception) { agent = ""; }

                            DateTime enddate = DateTime.Parse("2022-10-31");
                            //add thong tin rows vao product
                            var cate = new VOUCHER_API()
                            {
                                CODE = code,
